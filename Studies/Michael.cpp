...
SCDLLName("Michael Silver Studies");
...
SCSFExport scsf_ConsecutiveLongShortSales(SCStudyInterfaceRef sc)
{
	// Set up the study
	if (sc.SetDefaults)
	{
		sc.GraphName = "Consecutive Long/Short Sales";
		sc.AutoLoop = 1;

		// Define subgraphs for long and short sales alerts
		sc.Subgraph[0].Name = "Consecutive Long Sales";
		sc.Subgraph[0].DrawStyle = DRAWSTYLE_COLOR_BAR;
		sc.Subgraph[0].PrimaryColor = RGB(0, 255, 0);  // Green color
		sc.Subgraph[0].LineWidth = 2;

		sc.Subgraph[1].Name = "Consecutive Short Sales";
		sc.Subgraph[1].DrawStyle = DRAWSTYLE_COLOR_BAR;
		sc.Subgraph[1].PrimaryColor = RGB(255, 0, 0);  // Red color
		sc.Subgraph[1].LineWidth = 2;

		// Input for the number of consecutive sales
		sc.Input[0].Name = "Number of Consecutive Sales";
		sc.Input[0].SetInt(3);

		return;
	}

	// Variables to keep track of consecutive long and short sales
	static int LongSalesCount = 0;
	static int ShortSalesCount = 0;

	// Get the number of consecutive sales threshold from the input
	int ConsecutiveSalesThreshold = sc.Input[0].GetInt();

	// Determine if the current trade is a long or short sale
	if (sc.LastTradePrice[sc.Index] > sc.LastTradePrice[sc.Index - 1])
	{
		// It's a long sale
		LongSalesCount++;
		ShortSalesCount = 0;  // Reset short sales count
	}
	else if (sc.LastTradePrice[sc.Index] < sc.LastTradePrice[sc.Index - 1])
	{
		// It's a short sale
		ShortSalesCount++;
		LongSalesCount = 0;  // Reset long sales count
	}
	else
	{
		// No change in price, reset both counts
		LongSalesCount = 0;
		ShortSalesCount = 0;
	}

	// Check if the count exceeds the threshold and set the appropriate subgraph
	if (LongSalesCount >= ConsecutiveSalesThreshold)
	{
	...
	if (ShortSalesCount >= ConsecutiveSalesThreshold)
	{
		sc.Subgraph[1][sc.Index] = sc.Close[sc.Index];
	}
	else
	{
